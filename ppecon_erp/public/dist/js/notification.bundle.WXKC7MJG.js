(()=>{function v(r,e,p){let s="";if(p){let o=new Date;o.setTime(o.getTime()+p*24*60*60*1e3),s="; expires="+o.toUTCString()}document.cookie=r+"="+(e||"")+s+"; path=/"}function _(r){let e=r+"=",p=document.cookie.split(";");for(let s=0;s<p.length;s++){let o=p[s];for(;o.charAt(0)===" ";)o=o.substring(1,o.length);if(o.indexOf(e)===0)return o.substring(e.length,o.length)}return null}function N(r){let e=$("<div></div>"),p=new Audio("/files/pristine-609.mp3"),s=$(`
    ...
    `;$("<style>").text(x).appendTo("head");let m=JSON.parse(_("previousNotificationIds"))||[];l(d),setInterval(()=>l(d),1e4),y();function g(){console.log("Playing sound from:",p.src),p.play().catch(function(n){console.error("Audio playback failed:",n)})}function q(n){let i=$(`
        ...
        `);$("body").append(i),setTimeout(()=>{i.css({top:"20px",opacity:"1"})},100),setTimeout(()=>{i.css({top:"-50px",opacity:"0"}),setTimeout(()=>i.remove(),500)},4e3)}function l(n,i=!1){frappe.call({method:"ppecon_erp.notification.get_unread_notifications",callback:function(t){let c=t.message,u=$("#notification-list");if(u.empty(),c&&c.length>0){let f=c.map(a=>a.name);f.filter(a=>!m.includes(a)).length>0&&!i&&g(),m=f,v("previousNotificationIds",JSON.stringify(m),1),n.text(c.length),c.forEach(a=>{let k=b(new Date(a.creation)),h=$(`
                            <div class="notification-item" style="display: flex; align-items: center; padding: 12px; border-bottom: 1px solid #ddd; cursor: pointer;">
                                <div class="notification-content" style="flex-grow: 1; overflow: hidden;">
                                    ...
                                    <span class="notification-time" style="font-size: 11px; color: #888;">${k}</span>
                                </div>
                            </div>
                        `);h.on("click",async function(){await w(a.name),l(n,!0),window.location.href=`/app/${a.document_type.toLowerCase()}/${a.document_name}`}),u.append(h)})}else u.append('<p style="padding: 15px; text-align: center; color: #777;">No notifications available.</p>')}})}function b(n){let i=Math.floor((new Date-n)/1e3),t=Math.floor(i/31536e3);return t>1?t+" years ago":(t=Math.floor(i/2592e3),t>1?t+" months ago":(t=Math.floor(i/86400),t>1?t+" days ago":(t=Math.floor(i/3600),t>1?t+" hours ago":(t=Math.floor(i/60),t>1?t+" minutes ago":Math.floor(i)+" seconds ago"))))}function w(n){return frappe.call({method:"ppecon_erp.notification.mark_notification_as_read",args:{notification_name:n},callback:function(i){l($(".count"),!0)}})}function y(){window.addEventListener("quick_chat_child__close_dialog",function(n){$("#quick-chat-popup").modal("hide")},!1)}}N(document.querySelector(".main-section"));})();
//# sourceMappingURL=notification.bundle.WXKC7MJG.js.map
